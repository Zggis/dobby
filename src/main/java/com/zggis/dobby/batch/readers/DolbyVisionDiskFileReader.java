...
public class DolbyVisionDiskFileReader implements ItemReader<String> {

	private static final Logger logger = LoggerFactory.getLogger(DolbyVisionDiskFileReader.class);
	...
	Stack<String> dolbyVisionFileNames = new Stack<>();

	public DolbyVisionDiskFileReader(String mediaDir) {
				...
				if (child.getName().endsWith(".mp4")) {
					...
					if (newFile.getSeason() != null && newFile.getEpisode() != null) {
						if (newFile.isDolbyVision()) {
							...
							dolbyVisionFileNames.push(newFile.getFullName());
						}
					}
				}
			}
	...
	public String read() throws Exception, UnexpectedInputException, ParseException, NonTransientResourceException {
		if (!dolbyVisionFileNames.isEmpty()) {
			return dolbyVisionFileNames.pop();
		}
		return null;
	}
