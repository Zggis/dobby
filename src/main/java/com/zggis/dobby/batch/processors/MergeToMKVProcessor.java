...
public class MergeToMKVProcessor implements ItemProcessor<VideoMergeDTO, VideoFileDTO> {
  ...
  static {
    fpsMap = new HashMap<>();
    fpsMap.put(
        "23.976", "--default-duration 0:23.97602397602p --fix-bitstream-timing-information 0:1");
    fpsMap.put(
        DEFAULT_RESOLUTION, "--default-duration 0:24p --fix-bitstream-timing-information 0:1");
  ...
  public MergeToMKVProcessor(
      DoviProcessBuilder pbservice,
  ...
  public VideoFileDTO process(VideoMergeDTO merge) throws IOException {
    ...
    logger.info(
        "Generating MKV file from {}...", JobUtils.getWithoutPath(merge.getBlRPUFile().getName()));
    String cmd =
        MKVMERGE
            + " --output \""
